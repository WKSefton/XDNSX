...
export async function updateProject(token, project) {
    const operationsDoc = `
  query updateProject($id: Int!, $name: String!, $type: Int!, $data: jsonb!, $description: String!) {
    update_projects_by_pk(pk_columns: {id: $id}, _set: {name: $name, type: $type, data: $data, description: $description}) {
      data
      description
      id
      name
